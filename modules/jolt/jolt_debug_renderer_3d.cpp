...
void JoltDebugRenderer3D::draw(
	...
	camera_position = convert(p_camera.get_camera_transform().origin);
	settings = p_settings;
...
int32_t JoltDebugRenderer3D::submit(const RID &p_mesh) {
	...
	if (triangle_count > 0) {
		...
		RenderingServer::SurfaceData triangles_surface_data;
	...
	if (line_count > 0) {
		...
		RenderingServer::SurfaceData lines_surface_data;
...
void JoltDebugRenderer3D::DrawLine(JPH::RVec3Arg p_from, JPH::RVec3Arg p_to, JPH::Color p_color) {
	_reserve_lines(1);
	_add_line(convert_r(p_from), convert_r(p_to), convert(p_color).to_abgr32());
}
...
void JoltDebugRenderer3D::DrawGeometry(
	...
	if (camera_distance_sq > settings.camera_distance * settings.camera_distance) {
		// This Geometry is too far, don't draw.
		return;
	}
	...
	const auto *triangle_batch =
			static_cast<const JoltDebugTriangleBatch *>(model_lod->mTriangleBatch.GetPtr());
	...
	const uint32_t model_color_abgr = convert(p_model_color).to_abgr32();
	...
	if (p_draw_mode == JPH::DebugRenderer::EDrawMode::Solid) {
		...
		for (int32_t i = 0; i < model_triangle_count; ++i) {
			...
			const Vector3 v0 = convert_r(p_model_matrix * model_vertices_ptr[vertex_offset + 0]);
			const Vector3 v1 = convert_r(p_model_matrix * model_vertices_ptr[vertex_offset + 1]);
			const Vector3 v2 = convert_r(p_model_matrix * model_vertices_ptr[vertex_offset + 2]);
...
void JoltDebugRenderer3D::DrawText3D(
	...
	ERR_FAIL_MSG("Method not implemented.");
}

int64_t JoltDebugRenderer3D::get_max_vertex_count() const {
	return MIN(INT_MAX / DEBUG_VERTEX_STRIDE, INT_MAX / DEBUG_ATTRIBUTE_STRIDE);
}
...
void JoltDebugRenderer3D::_reserve_triangles(int32_t p_extra_capacity) {
	...
	const int64_t max_vertex_count = get_max_vertex_count();

	triangle_capacity += p_extra_capacity;
	triangle_capacity = MIN(triangle_capacity, max_vertex_count / 3);

	const int64_t vertex_count = (int64_t)triangle_capacity * 3;
	CRASH_COND((vertex_count * DEBUG_VERTEX_STRIDE) > INT_MAX);
	CRASH_COND((vertex_count * DEBUG_ATTRIBUTE_STRIDE) > INT_MAX);
	triangle_vertices.resize(vertex_count * DEBUG_VERTEX_STRIDE);
	triangle_attributes.resize(vertex_count * DEBUG_ATTRIBUTE_STRIDE);
}
...
void JoltDebugRenderer3D::_reserve_lines(int32_t p_extra_capacity) {
	...
	line_capacity = MIN(line_capacity, max_vertex_count / 2);
...
void JoltDebugRenderer3D::_add_triangle(
	...
	const int64_t max_triangle_count = get_max_vertex_count() / 3;

	if ((triangle_count + 1) > max_triangle_count) {
		// Limit reached.
		return;
	}
...
void JoltDebugRenderer3D::_add_line(
		...
		const Vector3 &p_to,
		uint32_t p_color_abgr) {
	const int64_t max_line_count = get_max_vertex_count() / 2;
	if ((line_count + 1) > max_line_count) {
		// Limit reached.
		return;
	}
